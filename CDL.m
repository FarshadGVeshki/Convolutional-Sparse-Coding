function [X,D, res] = CDL(D, S, lamb, opts)
% Efficient algorithm for onvolutional dictionary learning (CDL)
%
% Inputs:
...
while itr<=MaxIter && (r_csc > eprix || s_csc > eduax || r_cdl > eprid || s_cdl > eduad)
    ...
    for ttt = 1:cdl_iters % default = 1
        ...
        D = D_proj(sum(Gr+V,4)/P,m,H,W); % projection on constraint set
        if opts.dcfilter == 1
            D(:,:,1) = Dprv(:,:,1);
        end
        V = Gr - D + V;
